"""
O4_AI_Overlay.py — AI-based vegetation and building overlay generation for Ortho4XP.
...
# Set ai_building_model="landcover" to use CLASS_BUILDING from the veg model instead.
...
# Only used for large-footprint buildings (≥ ai_bld_obj_threshold_m2).
_FAC_DEFS = {
...
ai_overlay_enabled  = False   # master enable — set True in tile config to run
ai_do_vegetation    = True    # include vegetation classes in output
ai_do_buildings     = True    # include buildings class in output
ai_canvas_px        = 16384   # stitch resolution (0 = legacy per-DDS mode)
ai_patch_size       = 512     # pixels fed to the model per forward pass
ai_overlap          = 64      # pixel overlap between adjacent patches (blending)
ai_batch_size       = 0       # 0 = auto-size from available VRAM
ai_confidence_thr   = 0.5     # minimum softmax probability for a pixel to be assigned
ai_min_veg_area_px        = 200   # minimum vegetation blob area in pixels
ai_min_bld_area_px        = 15    # minimum building blob area in pixels
ai_bld_obj_threshold_m2   = 200   # footprint below this → SFD object; above → .fac facade
ai_building_model         = "dedicated"  # "dedicated" | "landcover"
...
def _stitch_dds_canvas(tex_dir, lat, lon, canvas_size):
    ...
    O4_Veg_Overlay._stitch_dds_textures.
    ...
    if pasted == 0:
        return None
    print(f"[AI Overlay] Stitched {pasted} DDS textures into {canvas_size}×{canvas_size} canvas")
...
def load_vegetation_model(device=None):
    ...
    print(f"[AI Overlay] Loading vegetation model ({_MODEL_VEG_REPO}) onto {device} …")
    _processor_veg = SegformerImageProcessor(
        do_resize=True,
        size={"height": ai_patch_size, "width": ai_patch_size},
        do_normalize=True,
    ...
    print("[AI Overlay] Vegetation model ready.")
    return _model_veg, _processor_veg, device
...
def load_building_model(device=None):
    ...
    print(f"[AI Overlay] Loading building model ({_MODEL_BLD_REPO}) onto {device} …")
    ...
    print("[AI Overlay] Building model ready.")
    return _model_bld, _processor_bld, device
...
def _infer_batch_size(device, patch_size, num_classes):
    ...
    Sizes the batch to consume at most 50% of available GPU VRAM.
    Falls back to 1 on CPU or if the query fails.
    """
    import torch
    if ai_batch_size and ai_batch_size > 0:
        return max(1, int(ai_batch_size))
    ...
    try:
        ...
        return max(1, min(64, int(free_bytes * 0.5 / bytes_per_patch)))
    except Exception:
        return 1
...
def _autotune_batch_size(model, device, sample_patch, max_batch_size, patch_size, num_classes, mean, std):
    ...
    if ai_batch_size and ai_batch_size > 0:
        print(f"[AI Overlay] Inference batch size: {max_batch_size} (configured)")
    ...
    # ai_batch_size still lets the user force a larger value.
    ...
    for n in candidates:
        try:
            print(f"[AI Overlay] Autotune batch {n}/{max_batch_size} …", flush=True)
            ...
            print(f"[AI Overlay] Autotune batch {n}: {rate:.1f} patches/s", flush=True)
            if rate > best_rate:
                best_rate = rate
                best_n = n
            ...
            if _is_cuda_oom(exc):
                ...
                print(f"[AI Overlay] Autotune batch {n}: OOM", flush=True)
                torch.cuda.empty_cache()
                continue
            raise
        except Exception as exc:
            results.append(f"{n}:err")
            print(f"[AI Overlay] Batch autotune skipped candidate {n}: {exc}", flush=True)
        finally:
            torch.cuda.empty_cache()
    ...
    if results:
        print(f"[AI Overlay] Inference batch autotune: {'  '.join(results)}  -> {best_n}", flush=True)
    return best_n
...
def run_inference(model, device, img_rgb, processor=None):
    ...
    Patches overlap by ai_overlap pixels on every side; their soft (softmax)
    ...
    if device.type == 'cuda':
        ...
        try:
            ...
            if _is_cuda_oom(exc):
                print("[AI Overlay] GPU blend accumulator OOM; falling back to CPU blending.")
                torch.cuda.empty_cache()
    ...
    if device.type == 'cuda':
        ...
        print(f"[AI Overlay] Inference batch size: {batch_size}")
...
def write_text_dsf(lat, lon, txt_path, veg_polys_by_class, bld_polys):
    ...
    Small-footprint buildings (< ai_bld_obj_threshold_m2) are placed as
    ...
    for raw in bld_polys:
        for poly in _iter_polygons(raw):
            ...
            if area < ai_bld_obj_threshold_m2:
...
def compile_dsf(txt_path, dsf_path):
    ...
    if result.returncode != 0:
        print(f"[AI Overlay] DSFTool error:\n{result.stdout}\n{result.stderr}")
        return False
    return True
...
def _run_inference_on_canvas(img_rgb, model, processor, device):
    ...
    print(f"[AI Overlay] Running inference on {img_rgb.shape[1]}×{img_rgb.shape[0]} canvas …")
    return run_inference(model, device, img_rgb, processor)


def process_tile(lat, lon, build_dir=None, device=None,
                 do_vegetation=None, do_buildings=None):
    """
    Full pipeline for one Ortho4XP tile (lat, lon).

    When ai_building_model == "dedicated":
    ...
    When ai_building_model == "landcover":
    ...
    if do_vegetation is None:
        do_vegetation = ai_do_vegetation
    if do_buildings is None:
        do_buildings = ai_do_buildings

    use_dedicated_bld = (do_buildings and ai_building_model == "dedicated")
    use_landcover_bld = (do_buildings and ai_building_model != "dedicated")
    ...
    if not os.path.isdir(tex_dir):
        print(f"[AI Overlay] No textures directory found: {tex_dir}")
        return None

    dds_files = [f for f in os.listdir(tex_dir) if f.lower().endswith(".dds")]
    if not dds_files:
        print(f"[AI Overlay] No DDS files in {tex_dir}")
        return None

    device = _get_device(device)
    all_veg_by_class = {}
    all_bld_polys    = []
    canvas_size      = ai_canvas_px  # 0 = legacy per-DDS mode
    ...
    if canvas_size > 0:
        ...
        if img_rgb is None:
            print("[AI Overlay] Could not stitch DDS textures.")
            return None
        ...
        if do_vegetation or use_landcover_bld:
            ...
            if use_landcover_bld:
                bld = mask_to_polygons(
                    ...
                    min_area_px=max(ai_min_bld_area_px, int(10 / m2_per_px))
                )
        ...
        if use_dedicated_bld:
            print("[AI Overlay] Building pass …")
        ...
        for fname in sorted(dds_files):
            ...
            print(f"[AI Overlay]   Processing {fname} …")
            try:
                img_rgb = _load_dds(fpath)
            except Exception as e:
                print(f"[AI Overlay]   Could not load {fname}: {e}")
                continue
    ...
    if not all_veg_by_class and not all_bld_polys:
        print("[AI Overlay] No features detected — no DSF written.")
        return None
    ...
    print(f"[AI Overlay] Writing DSF text to {txt_path} …")
    write_text_dsf(lat, lon, txt_path, all_veg_by_class, all_bld_polys)

    print(f"[AI Overlay] Compiling DSF → {dsf_path} …")
    ...
    print(f"[AI Overlay] Done. Overlay at:\n  {dsf_path}")
    return dsf_path
